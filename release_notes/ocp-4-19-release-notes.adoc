...
Starting with {product-title} 4.19, the perspectives in the web console have unified to simplify navigation, reduce context switching, streamline tasks, and provide users with a more cohesive {product-title} experiance.

With this unified design, there is no longer a *Developer* perspective by default; however,
_all_ {product-title} web console features are discoverable to all users. If you are not the cluster owner, you might need to request permission to certain features from the cluster owner. You can still enable the *Developer* perspective.
//See <module> for more information on enabling the *Developer* perspective. A quick start is also available with more information.

The *Getting Started* card in the web console also provides resources such as, a tour of the console, information on setting up your cluster, and links to explore new features and capabilities.
...
==== Cluster API replaces Terraform for {ibm-cloud-title} installations
