...
* A *Start Job* option was added to the *List* and *Details* pages, so you can start individual CronJobs manually directly in the web console without having to use the `oc` CLI.
* The *Import YAML* button in the masthead is now a *Quick Create* button, and to allow rapid deployment of workloads, you can use YAML, Git, or a container image.
...
Additionally, you can report any plugins that break security policies. Administrators have the ability to disable any plugin breaking those policies. It is deployed in report-only mode. CSP violations will be logged in the browser console, but the associated CSP directives will not be enforced. This feature is behind a `feature-gate`, so you will need to manually enable it.
