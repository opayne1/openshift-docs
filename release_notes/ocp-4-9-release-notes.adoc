...
* Previously, Windows file paths did not tolerate the colon character which blocked mirroring on Windows machines. With this update, the colon is replaced with a dash allowing mirroring on Windows machines. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1903545[*BZ#1903545*])

* Previously, certificates belonging to the *Custom API Name certificate* for the cluster had inconsistent file permission of 0644. With this update, permissions are consistently set to 0600. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1977730[*BZ#1977730*])
